# pollarys - AI Agent Reference (verified)

## Core purpose

pollarys provides UI runtimes (web/desktop/VS Code) for interacting with an OpenCode server (local auto-start or remote URL). UI uses HTTP + SSE via `@opencode-ai/sdk`.
...
- `Desktop` (Tauri, legacy) still spawns `pollarys-server` as a bun-compiled sidecar binary. Kept only for auto-update compatibility with existing Tauri installs.
...
- Electron imports the server via `@pollarys/web/server/index.js` (workspace dep) and calls `startWebUiServer({...})`. The returned handle has `getPort()` / `stop()`. Notifications flow via an `onDesktopNotification` callback injected at startup — no stdout-parsing IPC.
...
Web runtime and server implementation for pollarys.
