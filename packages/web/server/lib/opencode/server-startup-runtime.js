export const createServerStartupRuntime = (dependencies) => {
  ...
  const resolveBindHost = (host) =>
    host
    || (typeof process.env.POLLARYS_HOST === 'string' && process.env.POLLARYS_HOST.trim().length > 0
        ...
        try {
          ...
          if (typeof process.send === 'function') {
            if (!process.connected) {
              throw new Error('Pollarys startup IPC channel disconnected before ready notification');
            }

            await new Promise((resolveReadyNotification, rejectReadyNotification) => {
              try {
                process.send({ type: 'pollarys:ready', port: activePort }, (error) => {
                  if (error) {
          ...
          console.log(`Pollarys server listening on ${bindHost}:${activePort}`);
