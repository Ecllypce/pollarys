export const registerPollarysRoutes = (app, dependencies) => {
  const {
    fs,
    os,
    path,
  ...
  app.get('/api/pollarys/update-check', async (req, res) => {
    try {
  ...
  app.post('/api/pollarys/update-install', async (_req, res) => {
    try {
      ...
      const instanceFilePath = path.join(tmpDir, `pollarys-${currentPort}.json`);
      ...
      let restartCmdFallback = `pollarys serve --port ${storedOptions.port}`;
      ...
      const updateLogPath = path.join(pollarysDataDir, 'update-install.log');
      const logPreamble = [
        '',
        `=== Pollarys update ${new Date().toISOString()} ===`,
      ...
      res.json({
        ...
        setTimeout(() => {
          ...
          const script = isWindows
            ...
            ${updateCmd}
            if %ERRORLEVEL% EQU 0 (
              echo Update successful, restarting Pollarys...
              ${restartCmd}
            ) else (
              echo Update failed
              exit /b 1
  ...
  app.get('/api/pollarys/models-metadata', async (_req, res) => {
    const now = Date.now();
