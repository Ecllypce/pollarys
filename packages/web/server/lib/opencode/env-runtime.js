...
export const createOpenCodeEnvRuntime = (deps) => {
  ...
  const resolveWslExecutablePath = () => {
    ...
    const explicit = [process.env.WSL_BINARY, process.env.POLLARYS_WSL_BINARY]
  ...
  const resolveNodeCliPath = () => {
    const explicit = [process.env.NODE_BINARY, process.env.POLLARYS_NODE_BINARY]
      .map((v) => (typeof v === 'string' ? v.trim() : ''))
      .filter(Boolean);
  ...
  const resolveBunCliPath = () => {
    const explicit = [process.env.BUN_BINARY, process.env.POLLARYS_BUN_BINARY]
      .map((v) => (typeof v === 'string' ? v.trim() : ''))
      .filter(Boolean);
  ...
  const createConfiguredOpencodeBinaryError = (raw, normalized) => {
    ...
    const messageSuffix = 'Pollarys needs the standalone opencode CLI. Install it and set settings.opencodeBinary to the CLI path, for example ~/.opencode/bin/opencode, or leave the setting empty to use PATH lookup.';
    const error = (() => {
  ...
  const createConfiguredWslOpencodeError = (raw) => new Error(
    `Configured settings.opencodeBinary uses WSL but Pollarys could not resolve a WSL OpenCode command: ${raw}. Ensure WSL is available and opencode is installed in the configured distro.`
  );
  ...
  const resolveGitBinaryForSpawn = () => {
    ...
    const explicit = [process.env.GIT_BINARY, process.env.POLLARYS_GIT_BINARY]
