...
- `packages/web/server/lib/opencode/bootstrap-runtime.js`: base app bootstrap runtime for status/auth/tts/notification/Pollarys route wiring.
...
- `packages/web/server/lib/opencode/pollarys-routes.js`: Pollarys update and models metadata route registration.
...
## Public exports (pollarys-routes.js)
- `registerPollarysRoutes(app, dependencies)`: registers Pollarys endpoints:
  - `GET /api/pollarys/update-check`
  - `POST /api/pollarys/update-install`
  - `GET /api/pollarys/models-metadata`
  - `GET /api/zen/models`
...
- Rate limit config: `POLLARYS_RATE_LIMIT_MAX_ATTEMPTS`, `POLLARYS_RATE_LIMIT_NO_IP_MAX_ATTEMPTS` env vars.
