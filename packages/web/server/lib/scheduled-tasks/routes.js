...
export const registerScheduledTaskRoutes = (app, dependencies) => {
  ...
  app.get('/api/pollarys/scheduled-tasks/status', async (_req, res) => {
    try {
  ...
  app.get('/api/pollarys/events', (req, res) => {
    ...
    const clients = getPollarysEventClients();
    clients.add(res);

    try {
      writeSseEvent(res, {
        type: 'pollarys:event-stream-ready',
        properties: {
