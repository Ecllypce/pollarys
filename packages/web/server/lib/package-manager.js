...
const PACKAGE_NAME = '@pollarys/web';
const PACKAGE_PATH_SEGMENTS = PACKAGE_NAME.split('/');
const NPM_REGISTRY_URL = `https://registry.npmjs.org/${PACKAGE_NAME}`;
const CHANGELOG_URL = 'https://raw.githubusercontent.com/btriapitsyn/pollarys/main/CHANGELOG.md';
let cachedDetectedPm = null;

function getSpawnSyncBaseOptions() {
  return process.platform === 'win32' ? { windowsHide: true } : {};
}
const UPDATE_CHECK_URL = process.env.POLLARYS_UPDATE_API_URL || 'https://api.pollarys.dev/v1/update/check';

function getPollarysConfigDir() {
  if (process.platform === 'win32') {
    const appData = process.env.APPDATA;
    if (appData) return path.join(appData, 'pollarys');
  }
...
function getOrCreateInstallId(scope = 'web') {
  const configDir = getPollarysConfigDir();
...
function getOwnedPackagePathsFromGlobalBins(pm) {
  ...
  for (const binDir of getGlobalBinDirs(pm)) {
    const binaryName = process.platform === 'win32' ? 'pollarys.cmd' : 'pollarys';
...
export function detectPackageManagerDetails() {
  ...
  const forcedPm = process.env.POLLARYS_PACKAGE_MANAGER?.trim();
...
function isPackageInstalledWith(pm) {
  try {
    ...
    return result.stdout.includes(PACKAGE_NAME) || result.stdout.includes('pollarys');
  } catch {
    return false;
  }
}
