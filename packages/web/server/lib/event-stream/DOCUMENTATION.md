...
## Purpose
This module contains the Pollarys message-stream WebSocket protocol and runtime bridge. It keeps the browser-facing WebSocket transport separate from the upstream OpenCode SSE transport.
...
- Pollarys still fetches OpenCode upstream event streams over SSE.
...
- Global synthetic events such as `pollarys:session-status`, `pollarys:session-activity`, `pollarys:notification`, and `pollarys:heartbeat` are preserved on the WS path, but heartbeat frames are emitted only while an upstream SSE stream is actively attached.
