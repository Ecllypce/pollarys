...
const createCommandTimeoutMs = () => {
  const raw = Number(process.env.POLLARYS_FS_EXEC_TIMEOUT_MS);
...
const resolveWorkspacePath = ({ targetPath, baseDirectory, path, os, normalizeDirectoryPath, pollarysUserConfigRoot }) => {
  ...
  if (isPathWithinRoot(resolved, pollarysUserConfigRoot, path, os)) {
    return { ok: true, base: path.resolve(pollarysUserConfigRoot), resolved };
  }
...
const resolveWorkspacePathFromContext = async ({ req, targetPath, resolveProjectDirectory, path, os, normalizeDirectoryPath, pollarysUserConfigRoot }) => {
...
const resolveReadPathFromContext = async ({ req, targetPath, resolveProjectDirectory, path, os, normalizeDirectoryPath, pollarysUserConfigRoot }) => {
