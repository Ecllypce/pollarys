...
- This module owns GitHub auth, Octokit access, repo resolution, and Pull Request status resolution for Pollarys.
...
- Auth storage: `~/.config/pollarys/github-auth.json`
- Writes are atomic and file mode is `0o600`.
- Client ID resolution order: `POLLARYS_GITHUB_CLIENT_ID` -> `settings.json` -> default.
- Scope resolution order: `POLLARYS_GITHUB_SCOPES` -> `settings.json` -> default.
...
- PR state is persisted in local storage under `pollarys.github-pr-status`.
