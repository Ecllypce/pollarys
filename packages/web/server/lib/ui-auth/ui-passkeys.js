...
const DEFAULT_RP_NAME = 'Pollarys';
...
const PASSKEY_STORE_FILE = path.join(POLLARYS_DATA_DIR, 'ui-passkeys.json');
...
export const createUiPasskeys = ({
  ...
  const beginRegistration = async (req, { label } = {}) => {
    ...
    const options = await generateRegistrationOptions({
      ...
      rpID,
      userID,
      userName: 'pollarys-ui',
      userDisplayName: 'Pollarys UI',
  ...
  const finishAuthentication = async (payload) => {
    ...
    if (!passkey) {
      const error = new Error('That passkey is not registered for this Pollarys instance');
      error.statusCode = 404;
      throw error;
    }
