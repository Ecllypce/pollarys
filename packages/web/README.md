...
Full project overview, screenshots, and all features: [github.com/btriapitsyn/pollarys](https://github.com/btriapitsyn/pollarys)

## Install

```bash
curl -fsSL https://raw.githubusercontent.com/btriapitsyn/pollarys/main/scripts/install.sh | bash
```

Or install manually: `bun add -g @pollarys/web` (or npm, pnpm, yarn).
...
```bash
pollarys                          # Start on port 3000
pollarys --port 8080              # Custom port
pollarys --ui-password secret     # Password-protect UI
pollarys tunnel help              # Tunnel lifecycle commands
pollarys tunnel providers         # Show provider capabilities
pollarys tunnel profile add --provider cloudflare --mode managed-remote --name prod-main --hostname app.example.com --token <token>
pollarys tunnel start --profile prod-main
pollarys tunnel start --provider cloudflare --mode quick --qr
pollarys tunnel start --provider cloudflare --mode managed-local --config ~/.cloudflared/config.yml
pollarys tunnel status --all      # Show tunnel state across instances
pollarys tunnel stop --port 3000  # Stop tunnel only (server stays running)
pollarys logs                     # Follow latest instance logs
OPENCODE_PORT=4096 OPENCODE_SKIP_START=true pollarys                    # Connect to external OpenCode server
OPENCODE_HOST=https://myhost:4096 OPENCODE_SKIP_START=true pollarys  # Connect via custom host/HTTPS
pollarys stop                     # Stop server
pollarys update                   # Update to latest version
```

### Tunnel behavior notes

- One active tunnel per running Pollarys instance (port).
...
```bash
OPENCODE_PORT=4096 OPENCODE_SKIP_START=true pollarys
OPENCODE_HOST=https://myhost:4096 OPENCODE_SKIP_START=true pollarys
```
...
| `POLLARYS_OPENCODE_HOSTNAME` | Bind hostname for managed OpenCode server (default: `127.0.0.1`, use `0.0.0.0` for LAN/remote access — trusted networks only) |
| `POLLARYS_HOST` | Bind hostname for the Pollarys web server (default: `127.0.0.1`; use `0.0.0.0` for LAN/remote access — trusted networks only) |
| `POLLARYS_VERBOSE_REQUEST_LOGS` | Set to `true` to log every HTTP request; disabled by default to keep user logs small |
| `POLLARYS_SKIP_API_COMPRESSION` | Set to `true` to disable gzip compression for `/api/*` responses |
| `POLLARYS_COMPRESS_API` | Set to `true` to force `/api/*` compression, or `false` to disable it. Desktop runtime disables API compression by default to reduce local sidecar CPU use |
...
```bash
POLLARYS_OPENCODE_HOSTNAME=0.0.0.0 pollarys --port 3000
```
...
environment:
  ...
  POLLARYS_TUNNEL_MODE: quick # quick | managed-remote | managed-local
  POLLARYS_TUNNEL_PROVIDER: cloudflare
```
...
```yaml
environment:
  POLLARYS_TUNNEL_MODE: managed-remote
  POLLARYS_TUNNEL_HOSTNAME: app.example.com
  POLLARYS_TUNNEL_TOKEN: <token>
```
...
```yaml
environment:
  POLLARYS_TUNNEL_MODE: managed-local
  POLLARYS_TUNNEL_CONFIG: /home/pollarys/.cloudflared/config.yml
```

Managed-local path note: `POLLARYS_TUNNEL_CONFIG` must use a container path under `/home/pollarys/...`. If the config file references `credentials-file`, ensure that JSON path is also mounted and reachable inside the container.

**Data directory:** mount `data/` for persistent storage. Ensure permissions:
```bash
mkdir -p data/pollarys data/opencode/share data/opencode/config data/ssh
...
```bash
pollarys             # Runs in background by default
pollarys stop        # Stop background server
```
...
**`~/.config/systemd/user/pollarys.service`**
```ini
[Unit]
Description=Pollarys Web Server
After=opencode.service

[Service]
Type=simple
ExecStart=pollarys serve --port 3000 --host 0.0.0.0 --ui-password your-password --foreground
...
systemctl --user enable --now opencode pollarys
```

`--host 0.0.0.0` is required to listen on all interfaces (the default is `127.0.0.1`). Use `--host <ip>` or `POLLARYS_HOST=<ip>` to bind to a specific interface instead.
