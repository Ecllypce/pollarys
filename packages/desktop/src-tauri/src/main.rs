...
fn dispatch_menu_action<R: tauri::Runtime>(app: &tauri::AppHandle<R>, action: &str) {
    let _ = app.emit("pollarys:menu-action", action);

    let event = serde_json::to_string("pollarys:menu-action")
        .unwrap_or_else(|_| "\"pollarys:menu-action\"".into());
...
fn dispatch_check_for_updates<R: tauri::Runtime>(app: &tauri::AppHandle<R>) {
    let _ = app.emit("pollarys:check-for-updates", ());

    let event = serde_json::to_string("pollarys:check-for-updates")
        .unwrap_or_else(|_| "\"pollarys:check-for-updates\"".into());
...
const GITHUB_BUG_REPORT_URL: &str =
    "https://github.com/btriapitsyn/pollarys/issues/new?template=bug_report.yml";
#[cfg(target_os = "macos")]
const GITHUB_FEATURE_REQUEST_URL: &str =
    "https://github.com/btriapitsyn/pollarys/issues/new?template=feature_request.yml";
#[cfg(target_os = "macos")]
...
fn quit_confirmation_message() -> String {
    ...
    if reasons.is_empty() {
        ...
        format!(
            "Pollarys detected {}. Quitting now will stop sidecar/background processes and may interrupt pending work.",
            reasons.join(", ")
        )
    }
}
...
fn dispatch_installed_apps_update(app: &tauri::AppHandle, apps: &[InstalledAppInfo]) {
    let event = serde_json::to_string("pollarys:installed-apps-updated")
        .unwrap_or_else(|_| "\"pollarys:installed-apps-updated\"".into());
...
fn icon_to_data_url(icon_path: &Path, app_name: &str) -> Option<String> {
    ...
    let timestamp = SystemTime::now()
        ...
        .unwrap_or(0);
    let tmp_path = env::temp_dir().join(format!("pollarys-icon-{sanitized}-{timestamp}.png"));
...
const SIDECAR_NAME: &str = "pollarys-server";
const SIDECAR_NOTIFY_PREFIX: &str = "[PollarysDesktopNotify] ";
...
/// `POLLARYS_SERVER_URL` (no config-based host entry).
...
/// trailing slash and the other does not (e.g. `POLLARYS_SERVER_URL`
...
struct DesktopUiInjectionState {
    ...
    /// the correct `__POLLARYS_DESKTOP_BOOT_OUTCOME__` for that window.
...
fn settings_file_path() -> PathBuf {
    if let Ok(dir) = env::var("POLLARYS_DATA_DIR") {
...
/// `window.__POLLARYS_DESKTOP_BOOT_OUTCOME__`.
...
async fn refresh_quit_risk_flags(local_base_url: &str) {
    ...
    let scheduled_url = format!("{trimmed}/api/pollarys/scheduled-tasks/status");
    let tunnel_url = format!("{trimmed}/api/pollarys/tunnel/status");
...
const CHANGELOG_URL: &str =
    "https://raw.githubusercontent.com/btriapitsyn/pollarys/main/CHANGELOG.md";
...
fn maybe_show_sidecar_notification(app: &tauri::AppHandle, payload: SidecarNotifyPayload) {
    ...
    let title = payload
        ...
        .unwrap_or_else(|| "Pollarys".to_string());
...
/// Kills any stale pollarys-server processes that may be lingering from
...
async fn spawn_local_server(app: &tauri::AppHandle) -> Result<String> {
    ...
    for var in [
        "POLLARYS_OPENCODE_PATH",
        "POLLARYS_OPENCODE_BIN",
        "OPENCODE_PATH",
        "OPENCODE_BINARY",
    ] {
    ...
    for candidate in candidates {
        ...
        let mut cmd = app
            ...
            .env("POLLARYS_HOST", sidecar_bind_host)
            .env("POLLARYS_DIST_DIR", dist_dir.clone())
            .env("POLLARYS_RUNTIME", "desktop")
            .env("POLLARYS_DESKTOP_NOTIFY", "true")
            .env("POLLARYS_SKIP_API_COMPRESSION", "true")
...
fn desktop_notify(
    ...
    let mut builder = app
        .notification()
        .builder()
        .title(payload.title.unwrap_or_else(|| "Pollarys".to_string()));
...
async fn desktop_download_and_install_update(
    ...
    update
        .download_and_install(
            ...
            || {
                let _ = app.emit("pollarys:update-progress", UpdateProgressEvent::Finished);
            },
        )
        .await
        .map_err(|err| err.to_string())?;
...
fn build_init_script(local_origin: &str, boot_outcome: Option<&DesktopBootOutcome>) -> String {
    ...
    let mut init_script = format!(
        "(function(){{try{{window.__POLLARYS_HOME__={home_json};window.__POLLARYS_MACOS_MAJOR__={macos_major};window.__POLLARYS_LOCAL_ORIGIN__={local_json};window.__POLLARYS_DESKTOP_BOOT_OUTCOME__={boot_outcome_json};}}catch(_e){{}}}})();"
    );
...
fn open_new_window(app: &tauri::AppHandle) {
    ...
    let env_target = std::env::var("POLLARYS_SERVER_URL")
        .ok()
...
mod tests {
    ...
    fn unique_settings_path(test_name: &str) -> PathBuf {
        let nanos = SystemTime::now()
            ...
            .expect("clock drift")
            .as_nanos();
        std::env::temp_dir().join(format!("pollarys-{test_name}-{nanos}-settings.json"))
    }
    ...
    #[tokio::test]
    async fn probe_returns_ok_for_valid_pollarys_health_payload() {
    ...
    async fn spawn_flaky_pollarys_health_server() -> String {
        use tokio::net::TcpListener;
    ...
    async fn wait_for_local_opencode_ready_retries_until_health_payload_is_ready() {
        let url = spawn_flaky_pollarys_health_server().await;
        tokio::time::sleep(std::time::Duration::from_millis(50)).await;
    ...
    fn startup_failure_init_script_contains_boot_outcome_json() {
        ...
        // It must also set __POLLARYS_DESKTOP_BOOT_OUTCOME__
        assert!(
            script.contains("__POLLARYS_DESKTOP_BOOT_OUTCOME__"),
            "init script must set __POLLARYS_DESKTOP_BOOT_OUTCOME__"
        );
    }
}
