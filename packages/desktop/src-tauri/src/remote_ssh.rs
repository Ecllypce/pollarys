...
const SSH_STATUS_EVENT: &str = "pollarys:ssh-instance-status";
...
pub struct DesktopSshAuthConfig {
    ...
    pub pollarys_password: Option<DesktopSshStoredSecret>,
}
...
pub struct DesktopSshInstance {
    ...
    #[serde(default)]
    pub remote_pollarys: DesktopSshRemoteOpenchamberConfig,
    #[serde(default)]
...
struct RemoteSystemInfo {
    pollarys_version: Option<String>,
...
fn settings_file_path() -> PathBuf {
    if let Ok(dir) = std::env::var("POLLARYS_DATA_DIR") {
...
fn spawn_master_process(
    ...
    if let Some(secret) = ssh_password.filter(|value| !value.trim().is_empty()) {
        command.env("POLLARYS_SSH_ASKPASS_VALUE", secret.trim());
    }
...
fn current_remote_pollarys_version(
    parsed: &DesktopSshParsedCommand,
    control_path: &Path,
) -> Option<String> {
    run_remote_command(
        parsed,
        control_path,
        "pollarys --version 2>/dev/null || true",
        DEFAULT_CONNECTION_TIMEOUT_SEC,
    )
    .ok()
...
fn install_pollarys_managed(
    ...
    match preferred {
        DesktopSshInstallMethod::Bun => {
            if has_bun {
                commands.push(format!("bun add -g @pollarys/web@{version}"));
            }
            if has_npm {
                commands.push(format!("npm install -g @pollarys/web@{version}"));
            }
        }
    ...
    Err(last_error.unwrap_or_else(|| anyhow!("Failed to install Pollarys on remote host")))
}
...
fn configured_pollarys_password(instance: &DesktopSshInstance) -> Option<&str> {
    instance
        .auth
        .pollarys_password
        .as_ref()
...
fn probe_remote_system_info(
    ...
    control_path: &Path,
    port: u16,
    pollarys_password: Option<&str>,
) -> Result<RemoteSystemInfo> {
    let auth_payload = if let Some(password) = pollarys_password {
        serde_json::to_string(&json!({ "password": password })).unwrap_or_else(|_| "{}".to_string())
    } else {
        "{}".to_string()
    };

    let auth_enabled = if pollarys_password.is_some() {
        "1"
    } else {
        "0"
    };
    ...
    if is_liveness_http_status(info_status) {
        if is_auth_http_status(info_status) {
            if pollarys_password.is_some() && auth_status != 200 {
                return Err(anyhow!(format!(
                    "Remote Pollarys requires UI authentication and configured password was rejected (auth status {auth_status})"
                )));
            }
            ...
            return Err(anyhow!(
                "Remote Pollarys requires UI authentication on /api/system/info; configure Pollarys UI password"
            ));
        }
        ...
        return Err(anyhow!(format!(
            "Remote Pollarys probe failed (info status {info_status}, health status {health_status})"
        )));
    }

    let mut info = serde_json::from_str::<RemoteSystemInfo>(&body).unwrap_or_default();
    if info.pollarys_version.is_none() {
        if let Ok(value) = serde_json::from_str::<Value>(&body) {
            info.pollarys_version = value
                .get("pollarysVersion")
                .and_then(Value::as_str)
                .map(|v| v.to_string());
            info.runtime = value
...
fn remote_server_running(
    ...
    probe_remote_system_info(parsed, control_path, port, pollarys_password).is_ok()
}
...
fn start_remote_server_managed(
    ...
    let mut env_prefix = "POLLARYS_RUNTIME=ssh-remote".to_string();
    ...
    {
        env_prefix.push(' ');
        env_prefix.push_str("POLLARYS_UI_PASSWORD=");
        env_prefix.push_str(&shell_quote(&secret));
    }
    let script = format!(
        "{env_prefix} pollarys serve --daemon --hostname 127.0.0.1 --port {desired_port}"
    );
    let output = run_remote_command(
        parsed,
        control_path,
...
fn wait_local_forward_ready(local_port: u16) -> Result<()> {
    ...
    Err(anyhow!(
        "Timed out waiting for forwarded Pollarys health"
    ))
}
...
impl DesktopSshManagerInner {
    ...
    fn disconnect_internal(&self, app: &AppHandle, id: &str, report_idle: bool) {
        ...
        if let Some(mut session) = self.sessions.lock().expect("ssh sessions mutex").remove(id) {
            if session.started_by_us
                && matches!(
                    session.instance.remote_pollarys.mode,
                    DesktopSshRemoteMode::Managed
                )
                && !session.instance.remote_pollarys.keep_running
            {
    ...
    fn ensure_remote_server(
        ...
        match instance.remote_pollarys.mode {
            DesktopSshRemoteMode::External => {
                let Some(port) = instance.remote_pollarys.preferred_port else {
                    return Err(anyhow!(
                        "External mode requires a preferred remote Pollarys port"
                    ));
                };
                self.set_status(
                    app,
                    ...
                    Some("Probing external Pollarys server".to_string()),
                    None,
                    None,
                    Some(port),
                    false,
                ...
                probe_remote_system_info(
                    ...
                    control_path,
                    port,
                    configured_pollarys_password(instance),
                )
                .map_err(|err| {
                    anyhow!(format!(
                        "External Pollarys server probe failed on configured remote port: {err}"
                    ))
                })?;
                Ok((port, false))
            }
            DesktopSshRemoteMode::Managed => {
                self.set_status(
                    ...
                    Some("Checking remote Pollarys installation".to_string()),
                    None,
                    None,
                    None,
                    false,
                ...
                let installed_version = current_remote_pollarys_version(parsed, control_path);
                if installed_version.is_none() {
                    self.set_status(
                        app,
                        &instance.id,
                        DesktopSshPhase::Installing,
                        Some("Installing Pollarys on remote host".to_string()),
                        None,
                        None,
                        None,
                        false,
                    ...
                    install_pollarys_managed(
                        parsed,
                        control_path,
                        &app_version,
                        &instance.remote_pollarys.install_method,
                    )?;
                    ...
                    self.set_status(
                        ...
                        Some(format!(
                            "Updating remote Pollarys from {} to {}",
                            installed_version
                                .clone()
                ...
                self.set_status(
                    ...
                    &instance.id,
                    DesktopSshPhase::ServerDetecting,
                    Some("Detecting managed Pollarys server".to_string()),
                    None,
                    None,
                    None,
                    false,
                ...
                let mut remote_port = instance.remote_pollarys.preferred_port;
                ...
                if remote_port.is_none() {
                    self.set_status(
                        ...
                        Some("Starting managed Pollarys server".to_string()),
                        None,
                        None,
                        None,
                        false,
                ...
                let Some(port) = remote_port else {
                    return Err(anyhow!("Failed to determine remote Pollarys port"));
                };
                ...
                if !remote_server_running(
                    ...
                    return Err(anyhow!(
                        "Managed Pollarys server failed to become reachable"
                    ));
                }
...
mod tests {
    ...
    fn sample_instance(id: &str, command: &str) -> DesktopSshInstance {
        DesktopSshInstance {
            ...
            remote_pollarys: DesktopSshRemoteOpenchamberConfig::default(),
    ...
    fn parse_ssh_config_candidates_extracts_host_entries() {
        let temp =
            std::env::temp_dir().join(format!("pollarys-ssh-import-{}.txt", now_millis()));
        fs::write(
            &temp,
            "\nHost prod\n  HostName 10.0.0.1\nHost *.dev !skip\nHost *\n",
        )
