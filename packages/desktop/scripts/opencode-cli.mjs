...
const CLI_ARGS_ENV = process.env.POLLARYS_OPENCODE_ARGS;
...
async function resolveCliPath() {
  ...
  throw new Error('Unable to locate the OpenCode CLI. Set POLLARYS_OPENCODE_PATH to the executable.');
}
...
function spawnCli(cliPath, args) {
  const env = {
    ...process.env,
    POLLARYS_OPENCODE_PORT: process.env.POLLARYS_OPENCODE_PORT || process.env.OPENCODE_PORT || process.env.POLLARYS_INTERNAL_PORT || '0',
  };
  const cwd = process.env.POLLARYS_OPENCODE_CWD || process.cwd();
