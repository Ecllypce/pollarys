...
This package is docs content source-of-truth for Pollarys.
...
2. Add frontmatter at top:
   ...
   ---
   title: Remote Access
   description: Access Pollarys from outside your local network.
   ---
   ```
...
## Sync into pollarys-website

`pollarys-website` renders/deploys docs via Starlight in `apps/docs`.

After docs content updates here:

1. copy `packages/docs/content/docs/*` -> `pollarys-website/apps/docs/src/content/docs/*`
2. map `packages/docs/sidebar.config.json` into `pollarys-website/apps/docs/astro.config.mjs` sidebar
