...
Rendering and hosting still happen in `pollarys-website` (`apps/docs`).
...
- creates `pollarys-docs-source-<sha>.tar.gz`
...
The workflow can trigger a `repository_dispatch` event in `pollarys-website`.

Set secret in this repo:

- `POLLARYS_WEBSITE_REPO_TOKEN` (token with access to `pollarys/pollarys-website`)
...
`pollarys-website` can listen for this event and pull docs source from release artifacts.
