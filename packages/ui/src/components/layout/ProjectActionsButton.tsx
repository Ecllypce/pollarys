...
const AUTO_DISCOVER_ACTION_ID = '__POLLARYS_auto_discover_preview__';
...
export const ProjectActionsButton = ({
  ...
  const [actions, setActions] = React.useState<PollarysProjectAction[]>([]);
  ...
  const autoDiscoverAction = React.useMemo<PollarysProjectAction>(() => ({
  ...
  const getOrCreateActionTab = React.useCallback(async (action: PollarysProjectAction, options: { revealTerminal?: boolean } = {}) => {
  ...
  const runAction = React.useCallback(async (action: PollarysProjectAction) => {
    ...
    try {
      const discovered = action.id === AUTO_DISCOVER_ACTION_ID
        ? await (async (): Promise<PollarysProjectAction> => {
  ...
  const stopAction = React.useCallback(async (action: PollarysProjectAction) => {
    const runKey = toProjectActionRunKey(normalizedDirectory, action.id);
  ...
  const handleSelectAction = React.useCallback((action: PollarysProjectAction, toggleStopIfRunning = false) => {
