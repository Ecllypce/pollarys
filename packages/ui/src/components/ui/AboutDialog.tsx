    ...
    const fetchVersion = async () => {
      try {
        ...
        if (response.ok) {
          ...
          if (typeof data.PollarysVersion === 'string' && data.PollarysVersion.trim()) {
            setVersion(data.PollarysVersion);
            return;
          }
        }
      } catch {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-xs p-6">
        <div className="flex flex-col items-center text-center space-y-4">
          <PollarysLogo width={64} height={64} />

          <div className="space-y-1">
            <h2 className="text-lg font-semibold">Pollarys</h2>
            <div className="space-y-0.5 typography-meta text-muted-foreground">
              {displayVersion && (
                <p>{t('aboutDialog.PollarysVersionLabel', { version: displayVersion })}</p>
              )}
              {openCodeVersion && (
          ...
          <div className="flex flex-col items-center gap-2 pt-2">
            <div className="flex items-center justify-center gap-4">
              <a
                href="https://github.com/btriapitsyn/Pollarys"
                target="_blank"
            ...
            <a
              href="https://x.com/Pollarys_dev"
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-1.5 typography-meta text-muted-foreground hover:text-foreground transition-colors"
            >
              <Icon name="twitter-xfill" className="h-4 w-4" />
              <span>@Pollarys_dev</span>
            </a>
          </div>
