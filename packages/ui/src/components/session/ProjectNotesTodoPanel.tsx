...
import {
  ...
  Pollarys_PROJECT_NOTES_MAX_LENGTH,
  readProjectPlanFile,
  Pollarys_PROJECT_TODO_TEXT_MAX_LENGTH,
  saveProjectNotesAndTodos,
  type PollarysProjectPlanFileLink,
  type PollarysProjectTodoItem,
  type ProjectRef,
...
type ProjectPlanListItem = PollarysProjectPlanFileLink & {
  title: string;
};

const toPlanListItem = async (
  plan: PollarysProjectPlanFileLink,
...
export const ProjectNotesTodoPanel: React.FC<ProjectNotesTodoPanelProps> = ({
  ...
  const [todos, setTodos] = React.useState<PollarysProjectTodoItem[]>([]);
  ...
  const initializeNewPollarysSession = useSessionUIStore((state) => state.initializeNewPollarysSession);
  ...
  const persistProjectData = React.useCallback(
    async (nextNotes: string, nextTodos: PollarysProjectTodoItem[]) => {
      if (!projectRef) {
        return false;
      }
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('Pollarys:project-plan-saved', handleProjectContextRefresh);
    window.addEventListener('Pollarys:project-notes-updated', handleProjectContextRefresh);
    return () => {
      window.removeEventListener('Pollarys:project-plan-saved', handleProjectContextRefresh);
      window.removeEventListener('Pollarys:project-notes-updated', handleProjectContextRefresh);
    };
  }, [projectRef]);
  ...
  const handleAddTodo = React.useCallback(() => {
    ...
    const nextTodos = [
      ...
      {
        id: createTodoId(),
        text: trimmed.slice(0, Pollarys_PROJECT_TODO_TEXT_MAX_LENGTH),
        completed: false,
        createdAt: Date.now(),
      },
    ];
  ...
  const todoInputValue = newTodoText.slice(0, Pollarys_PROJECT_TODO_TEXT_MAX_LENGTH);
  ...
  const handleConfirmSend = React.useCallback(
    async (execution: TodoSendExecution) => {
      ...
      try {
        ...
        if (pendingSendTarget.kind === 'worktree') {
          ...
          initializeNewPollarysSession(session.id, useConfigStore.getState().agents ?? []);
        }
    ...
    [canCreateWorktree, createSession, initializeNewPollarysSession, onActionComplete, pendingSendTarget, projectRef, routeToChat, sendMessage, setCurrentSession, t]
  );
  ...
  const handleDeletePlan = React.useCallback(
    async (planId: string) => {
      ...
      try {
        ...
        window.dispatchEvent(new CustomEvent('Pollarys:project-plan-saved', {
    ...
    <div className={cn('w-full min-w-0 space-y-3 p-3', className)}>
      <div className="space-y-1">
        <div className="flex items-center justify-between gap-2">
          ...
          <span className="typography-meta text-muted-foreground">{notes.length}/{Pollarys_PROJECT_NOTES_MAX_LENGTH}</span>
        </div>
        <Textarea
          value={notes}
          onChange={(event) => setNotes(event.target.value.slice(0, Pollarys_PROJECT_NOTES_MAX_LENGTH))}
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between gap-2">
          ...
          <span className="typography-meta text-muted-foreground">{todoInputValue.length}/{Pollarys_PROJECT_TODO_TEXT_MAX_LENGTH}</span>
        </div>

        <div className="flex items-center gap-1.5">
          <Input
            value={todoInputValue}
            onChange={(event) => setNewTodoText(event.target.value.slice(0, Pollarys_PROJECT_TODO_TEXT_MAX_LENGTH))}
            onKeyDown={(event) => {
              if (event.key === 'Enter') {
                event.preventDefault();
                handleAddTodo();
