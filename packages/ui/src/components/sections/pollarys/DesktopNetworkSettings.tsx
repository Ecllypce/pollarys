...
export const DesktopNetworkSettings: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error(t('settings.pollarys.desktopNetwork.error.loadFailed'));
        }
        ...
        if (!cancelled) {
          setError(cause instanceof Error ? cause.message : t('settings.pollarys.desktopNetwork.error.loadFailed'));
        }
      } finally {
        if (!cancelled) {
          setIsLoading(false);
  ...
  const handleSaveAndRestart = React.useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(t('settings.pollarys.desktopNetwork.error.saveFailed'));
      }
      ...
      if (!restarted) {
        throw new Error(t('settings.pollarys.desktopNetwork.error.savedRestartFailed'));
      }
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : t('settings.pollarys.desktopNetwork.error.saveFailed'));
      setIsSaving(false);
    }
  ...
  return (
    <div className="mb-8">
      <div className="mb-1 px-1">
        <h3 className="typography-ui-header font-medium text-foreground">{t('settings.pollarys.desktopNetwork.title')}</h3>
      </div>
      ...
      <section className="space-y-2 px-2 pb-2 pt-0">
        ...
        >
          <Checkbox
            ...
            ariaLabel={t('settings.pollarys.desktopNetwork.field.allowLanAccessAria')}
            disabled={isLoading || isSaving}
          />
          <div className="min-w-0 flex-1">
            <div className="typography-ui-label text-foreground">{t('settings.pollarys.desktopNetwork.field.allowLanAccess')}</div>
            <div className="typography-micro text-muted-foreground/70">
              {t('settings.pollarys.desktopNetwork.field.allowLanAccessDescription')}
            </div>
            <div className="typography-micro text-[var(--status-warning)]/85">
              {t('settings.pollarys.desktopNetwork.field.warning')}
            </div>
          </div>
        </div>
        ...
        {lanUrl ? (
          <div className="px-2 typography-micro text-muted-foreground/80">
            {isDirty && !savedValue
              ? t('settings.pollarys.desktopNetwork.hint.openAfterRestart')
              : t('settings.pollarys.desktopNetwork.hint.openNow')}
        ...
        <div className="flex justify-start py-1.5">
          ...
          >
            {isSaving ? t('settings.common.actions.saving') : t('settings.pollarys.desktopNetwork.actions.saveAndRestart')}
          </Button>
        </div>
      </section>
    </div>
