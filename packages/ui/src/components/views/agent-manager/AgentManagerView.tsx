...
export const AgentManagerView: React.FC<AgentManagerViewProps> = ({ className }) => {
  const isVSCodeRuntime = Boolean(
    (typeof window !== 'undefined'
      ? (window as unknown as { __POLLARYS_RUNTIME_APIS__?: { runtime?: { isVSCode?: boolean } } })
          .__POLLARYS_RUNTIME_APIS__?.runtime?.isVSCode
      : false)
  );
  const [connectionStatus, setConnectionStatus] = React.useState<'connecting' | 'connected' | 'error' | 'disconnected'>(
    () =>
      (typeof window !== 'undefined'
        ? (window as unknown as { __POLLARYS_CONNECTION__?: { status?: string } }).__POLLARYS_CONNECTION__?.status as
  ...
  React.useEffect(() => {
    ...
    const current =
      (typeof window !== 'undefined'
        ? (window as unknown as { __POLLARYS_CONNECTION__?: { status?: string } }).__POLLARYS_CONNECTION__?.status
