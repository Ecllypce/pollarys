...
const persistToLocalStorage = (settings: DesktopSettings) => {
  ...
  if (settings.homeDirectory) {
    ...
    // Electron's preload exposes __POLLARYS_HOME__ as a read-only
    ...
    try {
      window.__POLLARYS_HOME__ = settings.homeDirectory;
    } catch {
  ...
  if (typeof settings.pwaAppName === 'string') {
    ...
    if (normalized.length > 0) {
      localStorage.setItem('pollarys.pwaName', normalized);
    } else {
      localStorage.removeItem('pollarys.pwaName');
    }
  }
...
export const syncDesktopSettings = async (): Promise<void> => {
  ...
  const applySettings = async (settings: DesktopSettings) => {
    ...
    if (typeof window !== 'undefined') {
      window.dispatchEvent(new CustomEvent<DesktopSettings>('pollarys:settings-synced', { detail: settings }));
    }
  };
