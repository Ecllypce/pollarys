/**
 * Pollarys project-level configuration service.
 * Stores per-project settings in ~/.config/Pollarys/<projectId>.json.
 * Migrates from legacy <project>/.Pollarys/Pollarys.json.
 */
...
const CONFIG_FILENAME = 'Pollarys.json';
// LEGACY_PROJECT_CONFIG: legacy per-project config root inside repo.
const LEGACY_CONFIG_DIR = '.Pollarys';
const USER_PROJECTS_DIR_SEGMENTS = ['.config', 'Pollarys', 'projects'];
...
export interface PollarysConfig {
  projectPath?: string;
  'setup-worktree'?: string[];
  projectNotes?: string;
  projectTodos?: PollarysProjectTodoItem[];
  projectPlanFiles?: PollarysProjectPlanFileLink[];
  projectActions?: PollarysProjectAction[];
  projectActionsPrimaryId?: string;
}

export type PollarysProjectActionPlatform = 'macos' | 'linux' | 'windows';

export interface PollarysProjectAction {
  id: string;
  name: string;
  command: string;
  icon?: string | null;
  platforms?: PollarysProjectActionPlatform[];
  autoOpenUrl?: boolean;
  openUrl?: string;
  desktopOpenSshForward?: string;
}

export interface PollarysProjectActionsState {
  actions: PollarysProjectAction[];
  primaryActionId: string | null;
}

export interface PollarysProjectTodoItem {
  id: string;
  text: string;
  completed: boolean;
  createdAt: number;
...
export interface PollarysProjectPlanFileLink {
  id: string;
  path: string;
  createdAt: number;
}

export interface PollarysProjectPlanFile {
  title: string;
  body: string;
  raw: string;
  path: string;
...
export interface PollarysProjectNotesTodos {
  notes: string;
  todos: PollarysProjectTodoItem[];
}

export interface PollarysProjectContextData extends PollarysProjectNotesTodos {
  plans: PollarysProjectPlanFileLink[];
}

export const Pollarys_PROJECT_NOTES_MAX_LENGTH = 3000;
export const Pollarys_PROJECT_TODO_TEXT_MAX_LENGTH = 120;
export const Pollarys_PROJECT_ACTION_NAME_MAX_LENGTH = 80;
export const Pollarys_PROJECT_ACTION_COMMAND_MAX_LENGTH = 4000;
export const Pollarys_PROJECT_ACTION_OPEN_URL_MAX_LENGTH = 2000;
export const Pollarys_PROJECT_ACTION_DESKTOP_FORWARD_MAX_LENGTH = 300;
export const Pollarys_PROJECT_PLAN_TITLE_MAX_LENGTH = 160;

const Pollarys_ACTION_PLATFORM_SET = new Set<PollarysProjectActionPlatform>(['macos', 'linux', 'windows']);
...
const resolveHomeDirectory = async (): Promise<string | null> => {
  ...
  // In some runtimes, window.__POLLARYS_HOME__ can be workspace/project-root
...
const sanitizeProjectNotes = (value: unknown): string => {
  ...
  return trimToMaxLength(value, Pollarys_PROJECT_NOTES_MAX_LENGTH);
};

const sanitizeProjectTodoItems = (value: unknown): PollarysProjectTodoItem[] => {
  if (!Array.isArray(value)) {
    return [];
  }

  const sanitized: PollarysProjectTodoItem[] = [];
  for (const entry of value) {
    ...
    const text = trimToMaxLength(textRaw.trim(), Pollarys_PROJECT_TODO_TEXT_MAX_LENGTH);
    if (!id || !text) {
      continue;
    }
...
const sanitizeProjectPlanFileLinks = (value: unknown): PollarysProjectPlanFileLink[] => {
  if (!Array.isArray(value)) {
    return [];
  }

  const sanitized: PollarysProjectPlanFileLink[] = [];
...
const sanitizeProjectActionPlatforms = (value: unknown): PollarysProjectActionPlatform[] => {
  if (!Array.isArray(value)) {
    return [];
  }

  const unique: PollarysProjectActionPlatform[] = [];
  const seen = new Set<PollarysProjectActionPlatform>();
  for (const entry of value) {
    if (typeof entry !== 'string') {
      continue;
    }
    const normalized = entry.trim().toLowerCase() as PollarysProjectActionPlatform;
    if (!Pollarys_ACTION_PLATFORM_SET.has(normalized) || seen.has(normalized)) {
      continue;
    }
    seen.add(normalized);
...
const sanitizeProjectActions = (value: unknown): PollarysProjectAction[] => {
  if (!Array.isArray(value)) {
    return [];
  }

  const sanitized: PollarysProjectAction[] = [];
  const seenIds = new Set<string>();
  ...
  for (const entry of value) {
    ...
    const id = typeof record.id === 'string' ? record.id.trim() : '';
    const name = trimToMaxLength(typeof record.name === 'string' ? record.name.trim() : '', Pollarys_PROJECT_ACTION_NAME_MAX_LENGTH);
    const command = trimToMaxLength(typeof record.command === 'string' ? record.command.trim() : '', Pollarys_PROJECT_ACTION_COMMAND_MAX_LENGTH);
    ...
    const openUrl = trimToMaxLength(openUrlRaw, Pollarys_PROJECT_ACTION_OPEN_URL_MAX_LENGTH);
    ...
    const desktopOpenSshForward = trimToMaxLength(
      ...
      Pollarys_PROJECT_ACTION_DESKTOP_FORWARD_MAX_LENGTH
    );
...
} | null | undefined): PollarysProjectActionsState => {
...
const sanitizeProjectNotesAndTodos = (value: {
  notes?: unknown;
  todos?: unknown;
} | null | undefined): PollarysProjectNotesTodos => {
  return {
...
const sanitizeProjectContextData = (value: {
  ...
  todos?: unknown;
  plans?: unknown;
} | null | undefined): PollarysProjectContextData => {
...
const sanitizePlanTitle = (value: string): string => {
  return trimToMaxLength(value.trim(), Pollarys_PROJECT_PLAN_TITLE_MAX_LENGTH);
};
 ...
 */
export async function readPollarysConfig(project: ProjectRef): Promise<PollarysConfig | null> {
  const projectDirectory = typeof project?.path === 'string' ? project.path.trim() : '';
  if (!projectDirectory) {
    return null;
  }
  ...
  const parseConfig = (text: string | null): PollarysConfig | null => {
    ...
    try {
      ...
      return parsed as PollarysConfig;
    } catch {
      return null;
    }
  };
  ...
  // 2) Migrate legacy <project>/.Pollarys/Pollarys.json.
  // LEGACY_PROJECT_CONFIG: migrate project-local Pollarys.json -> ~/.config/Pollarys/projects/<projectId>.json
  ...
  try {
    const wrote = await writePollarysConfig(project, legacyConfig);
    if (wrote) {
      await deleteLegacyPollarysConfig(projectDirectory);
    }
  } catch {
 ...
 */
export async function writePollarysConfig(
  project: ProjectRef,
  config: PollarysConfig
): Promise<boolean> {
  const projectDirectory = typeof project?.path === 'string' ? project.path.trim() : '';
  if (!projectDirectory) {
    return false;
  ...
  try {
    ...
    console.error('Failed to write Pollarys config:', error);
    return false;
  }
}
 ...
 */
export async function updatePollarysConfig(
  project: ProjectRef,
  updates: Partial<PollarysConfig>
): Promise<boolean> {
  const existing = await readPollarysConfig(project) || {};
  const merged = { ...existing, ...updates };
  return writePollarysConfig(project, merged);
}
...
export async function getWorktreeSetupCommands(project: ProjectRef): Promise<string[]> {
  const config = await readPollarysConfig(project);
...
export async function saveWorktreeSetupCommands(project: ProjectRef, commands: string[]): Promise<boolean> {
  ...
  return updatePollarysConfig(project, { 'setup-worktree': filtered });
}

export async function getProjectNotesAndTodos(project: ProjectRef): Promise<PollarysProjectNotesTodos> {
  const config = await readPollarysConfig(project);
...
export async function saveProjectNotesAndTodos(
  project: ProjectRef,
  value: PollarysProjectNotesTodos
): Promise<boolean> {
  ...
  return updatePollarysConfig(project, {
    projectNotes: sanitized.notes,
    projectTodos: sanitized.todos,
  });
}

export async function getProjectContextData(project: ProjectRef): Promise<PollarysProjectContextData> {
  const config = await readPollarysConfig(project);
...
export async function getProjectPlanFiles(project: ProjectRef): Promise<PollarysProjectPlanFileLink[]> {
  const config = await readPollarysConfig(project);
...
export async function saveProjectPlanFiles(
  project: ProjectRef,
  value: PollarysProjectPlanFileLink[]
): Promise<boolean> {
...
export async function readProjectPlanFile(path: string): Promise<PollarysProjectPlanFile | null> {
...
): Promise<PollarysProjectPlanFileLink | null> {
...
export async function getProjectActionsState(project: ProjectRef): Promise<PollarysProjectActionsState> {
  const config = await readPollarysConfig(project);
...
export async function saveProjectActionsState(
  project: ProjectRef,
  value: PollarysProjectActionsState
): Promise<boolean> {
...
async function deleteLegacyPollarysConfig(projectDirectory: string): Promise<void> {
  const legacyPath = getLegacyConfigPath(projectDirectory);
  const runtimeFiles = getRuntimeFilesAPI();
