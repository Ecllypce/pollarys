...
const localOrigin = readArgValue('--pollarys-local-origin');
const homeDirectory = readArgValue('--pollarys-home');
const macosMajorRaw = readArgValue('--pollarys-macos-major');
...
//  - __POLLARYS_ELECTRON__ is a shell-identity flag (no capability).
...
// Remote pages need __POLLARYS_LOCAL_ORIGIN__ so the HostSwitcher knows
...
if (localOrigin) {
  contextBridge.exposeInMainWorld('__POLLARYS_LOCAL_ORIGIN__', localOrigin);
}
...
if (isLocalPage && homeDirectory) {
  contextBridge.exposeInMainWorld('__POLLARYS_HOME__', homeDirectory);
}
...
if (Number.isFinite(macosMajor) && macosMajor > 0) {
  contextBridge.exposeInMainWorld('__POLLARYS_MACOS_MAJOR__', macosMajor);
}

contextBridge.exposeInMainWorld('__POLLARYS_ELECTRON__', {
  runtime: 'electron',
});
...
ipcRenderer.on('pollarys:emit', (_evt, payload) => {
...
// ipcMain.handle('pollarys:invoke') decides per-command what is safe
// for non-local callers (window/host-switcher ops yes, file/shell ops
// no). See COMMANDS_SAFE_FOR_REMOTE in main.mjs.
contextBridge.exposeInMainWorld('__TAURI__', {
  core: {
    invoke: (cmd, args) => ipcRenderer.invoke('pollarys:invoke', cmd, args || {}),
  },
  dialog: {
    open: (options) => ipcRenderer.invoke('pollarys:dialog:open', options || {}),
  },
  shell: {
    open: (url) => ipcRenderer.invoke('pollarys:invoke', 'desktop_open_external_url', { url }),
  },
  event: {
