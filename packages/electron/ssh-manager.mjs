...
const SSH_STATUS_EVENT = 'pollarys:ssh-instance-status';
...
if [[ -n "$POLLARYS_SSH_ASKPASS_VALUE" ]]; then
  if [[ "$PROMPT" == *"assword"* || "$PROMPT" == *"passphrase"* ]]; then
    printf '%s\\n' "$POLLARYS_SSH_ASKPASS_VALUE"
    exit 0
  fi
fi
...
const waitLocalForwardReady = async (localPort) => {
  ...
  throw new Error('Timed out waiting for forwarded Pollarys health');
};
...
export class ElectronSshManager {
  ...
  sanitizeInstance(instance) {
    ...
    return {
      ...
      auth: {
        ...
        ...(this.sanitizeStoredSecret(instance?.auth?.pollarysPassword) ? { pollarysPassword: this.sanitizeStoredSecret(instance.auth.pollarysPassword) } : {}),
      },
      portForwards,
    };
  }
  ...
  async spawnMasterProcess(parsed, controlPath, askpassPath, sshPassword) {
    const child = spawn('ssh', buildSshArgs(parsed, [
      ...
      env: {
        ...
        DISPLAY: '1',
        ...(sshPassword ? { POLLARYS_SSH_ASKPASS_VALUE: sshPassword.trim() } : {}),
      },
    });
    return child;
  }
  ...
  configuredPollarysPassword(instance) {
    const secret = instance?.auth?.pollarysPassword;
  ...
  async currentRemotePollarysVersion(parsed, controlPath) {
    try {
      const output = await runRemoteCommand(parsed, controlPath, 'pollarys --version 2>/dev/null || true');
  ...
  async installPollarysManaged(parsed, controlPath, version, preferred) {
    ...
    if (preferred === 'bun') {
      if (hasBun) commands.push(`bun add -g @pollarys/web@${version}`);
      if (hasNpm) commands.push(`npm install -g @pollarys/web@${version}`);
    ...
    throw lastError || new Error('Failed to install Pollarys on remote host');
  }

  async probeRemoteSystemInfo(parsed, controlPath, port, pollarysPassword) {
    const authPayload = pollarysPassword ? JSON.stringify({ password: pollarysPassword }) : '{}';
    const authEnabled = pollarysPassword ? '1' : '0';
    ...
    if (isLivenessHttpStatus(infoStatus)) {
      if (isAuthHttpStatus(infoStatus)) {
        if (pollarysPassword && authStatus !== 200) {
          throw new Error(`Remote Pollarys requires UI authentication and configured password was rejected (auth status ${authStatus})`);
        }
        if (isLivenessHttpStatus(healthStatus)) return {};
        throw new Error('Remote Pollarys requires UI authentication on /api/system/info; configure Pollarys UI password');
      }
    } else if (isLivenessHttpStatus(healthStatus)) {
      return {};
    } else {
      throw new Error(`Remote Pollarys probe failed (info status ${infoStatus}, health status ${healthStatus})`);
    }
  ...
  async remoteServerRunning(parsed, controlPath, port, pollarysPassword) {
    try {
      await this.probeRemoteSystemInfo(parsed, controlPath, port, pollarysPassword);
      return true;
    } catch {
      return false;
    }
  ...
  async startRemoteServerManaged(parsed, controlPath, instance, desiredPort) {
    let envPrefix = 'POLLARYS_RUNTIME=ssh-remote';
    const secret = this.configuredPollarysPassword(instance);
    if (secret) {
      envPrefix += ` POLLARYS_UI_PASSWORD=${shellQuote(secret)}`;
    }
    const output = await runRemoteCommand(parsed, controlPath, `${envPrefix} pollarys serve --hostname 127.0.0.1 --port ${desiredPort}`);
  ...
  async ensureRemoteServer(instance, parsed, controlPath) {
    if (instance.remoteOpenchamber.mode === 'external') {
      if (!instance.remoteOpenchamber.preferredPort) {
        throw new Error('External mode requires a preferred remote Pollarys port');
      }
      const port = instance.remoteOpenchamber.preferredPort;
      this.setStatus(instance.id, 'server_detecting', 'Probing external Pollarys server', null, null, port, false, 0, false);
      await this.probeRemoteSystemInfo(parsed, controlPath, port, this.configuredPollarysPassword(instance));
      return { remotePort: port, startedByUs: false };
    }

    this.setStatus(instance.id, 'remote_probe', 'Checking remote Pollarys installation');
    const installedVersion = await this.currentRemotePollarysVersion(parsed, controlPath);
    if (!installedVersion) {
      this.setStatus(instance.id, 'installing', 'Installing Pollarys on remote host');
      await this.installPollarysManaged(parsed, controlPath, this.appVersion, instance.remoteOpenchamber.installMethod);
    } else if (installedVersion !== this.appVersion) {
      this.setStatus(instance.id, 'updating', `Updating remote Pollarys from ${installedVersion} to ${this.appVersion}`);
      await this.installPollarysManaged(parsed, controlPath, this.appVersion, instance.remoteOpenchamber.installMethod);
    }

    this.setStatus(instance.id, 'server_detecting', 'Detecting managed Pollarys server');
    let remotePort = instance.remoteOpenchamber.preferredPort || null;
    let startedByUs = false;
    if (remotePort && !(await this.remoteServerRunning(parsed, controlPath, remotePort, this.configuredPollarysPassword(instance)))) {
      remotePort = null;
    }
    if (!remotePort) {
      this.setStatus(instance.id, 'server_starting', 'Starting managed Pollarys server');
    ...
    if (!(await this.remoteServerRunning(parsed, controlPath, remotePort, this.configuredPollarysPassword(instance)))) {
      throw new Error('Managed Pollarys server failed to become reachable');
    }
