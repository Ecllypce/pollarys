...
export function getWebviewHtml(options: WebviewHtmlOptions): string {
  ...
  // 
  // Logo geometry matches PollarysLogo.tsx:
...
<head>
  ...
  <title>Pollarys</title>
</head>
<body>
  <!-- Initial loading screen with simplified Pollarys logo -->
  ...
  <script>
    ...
    window.__POLLARYS_HOME__ = "${workspaceFolder.replace(/\\/g, '\\\\')}";
    ...
    if (!devServerUrl) {
      ...
      const tryLoadDevBundle = () => {
        ...
        Promise.resolve()
          ...
          .catch((error) => {
            attempt += 1;
            console.warn('[Pollarys] VS Code webview dev bundle unavailable, retrying...', error);
