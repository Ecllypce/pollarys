...
declare global {
  interface Window {
    __pollarysVsCodeStreamPerfState__?: StreamPerfState;
  }
}
...
const ensurePerfState = (): StreamPerfState | null => {
  ...
  if (!window.__pollarysVsCodeStreamPerfState__) {
    const startedAt = Date.now();
    window.__pollarysVsCodeStreamPerfState__ = {
  ...
  return window.__pollarysVsCodeStreamPerfState__;
};
