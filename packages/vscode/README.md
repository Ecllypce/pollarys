...
![VS Code Extension](https://github.com/btriapitsyn/pollarys/raw/HEAD/packages/vscode/extension.jpg)

**Like the extension? There's also a [desktop app and web version](https://github.com/btriapitsyn/pollarys) with even more features.**
...
Plus everything from the shared Pollarys UI: branchable timeline, smart tool UIs, voice mode, Git workflows, and more.

## Commands

| Command | Description |
|---------|-------------|
| `Pollarys: Focus Chat` | Focus the chat panel |
| `Pollarys: New Session` | Start a new chat session |
| `Pollarys: Open Sidebar` | Open the Pollarys sidebar |
| `Pollarys: Open Agent Manager` | Launch parallel multi-model runs |
| `Pollarys: Open Session in Editor` | Open current or new session in an editor tab |
| `Pollarys: Settings` | Open extension settings |
| `Pollarys: Restart API Connection` | Restart the OpenCode API process |
| `Pollarys: Show OpenCode Status` | Debug info for development or bug reports |

### Right-click menu

Select code in the editor, right-click, and find the **Pollarys** submenu:
...
|---------|---------|-------------|
| `pollarys.apiUrl` | _(empty)_ | URL of an external OpenCode API server. Leave empty to auto-start a local instance. |
| `pollarys.opencodeBinary` | _(empty)_ | Absolute path to the `opencode` CLI binary. Useful when PATH lookup fails. Requires window reload to apply. |
...
- `POLLARYS_VSCODE_BIN=cursor bun run vscode:dev`
- `POLLARYS_VSCODE_DEV_WORKSPACE=/path/to/workspace bun run vscode:dev`
...
Install locally: `code --install-extension packages/vscode/pollarys-*.vsix`
