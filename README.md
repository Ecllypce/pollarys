# <picture><source media="(prefers-color-scheme: dark)" srcset="docs/references/badges/pollarys-logo-dark.svg"><img src="docs/references/badges/pollarys-logo-light.svg" width="32" height="32" align="absmiddle" /></picture> pollarys
...
![pollarys Chat](docs/references/chat_example.png)
...
- **Connect to remote machines** - Desktop app connects to remote pollarys instances over SSH, with dedicated lifecycle and UX flows.
...
### **Desktop (macOS)**
Download from [Releases](https://github.com/btriapitsyn/pollarys/releases).

### **VS Code**
Install from [Marketplace](https://marketplace.visualstudio.com/items?itemName=fedaykindev.pollarys) or search "pollarys" in Extensions.
...
pollarys --ui-password be-creative-here
```
...
pollarys update                   # Update to latest
```
...
Run pollarys and OpenCode as separate persistent services — useful when you want to access your
...
- pollarys connects to it via `OPENCODE_HOST` and `--host 0.0.0.0` makes it reachable on your VPN IP.
...
pollarys will be reachable at `http://<your-vpn-hostname>:3000` from any device on your VPN.
...
> `POLLARYS_HOST=<ip>` to bind to a specific interface instead.
...
Managed-local path note: `POLLARYS_TUNNEL_CONFIG` must point to a path inside the container user home (`/home/pollarys/...`). If your Cloudflare config references a credentials JSON file, that file path must also be accessible inside the container (mount with `volumes`).
...
- pollarys supports one active tunnel per running instance (port).
...
- Connect to remote pollarys instances over SSH with dedicated lifecycle flows
...
- Custom themes via JSON files in `~/.config/pollarys/themes/` - hot reload, no restart
