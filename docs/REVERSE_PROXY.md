...
Use this guide when running Pollarys behind Nginx, Nginx Proxy Manager, Caddy, Cloudflare, or another reverse proxy.

## Before you proxy it

1. Confirm Pollarys works directly first.
...
- Disable gzip on the proxy if Pollarys is already compressing responses.
...
- Pollarys reachable directly on LAN
...
location ~ ^/api/(event|global/event|notifications/stream|Pollarys/events)$ {
    proxy_pass http://127.0.0.1:3000;
...
- Pollarys compresses HTTP responses with gzip (threshold 1 KB).
...
SSE streaming routes are excluded from compression by Pollarys, but the CDN may still buffer them. Check your CDN documentation for how to disable buffering on SSE paths.
