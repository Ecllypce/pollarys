...
/*
 * Port OpenCode themes into full Pollarys theme JSON files.
 *
 * Usage:
 *   bun run themes:port:opencode --list
 *   bun run themes:port:opencode github cursor lucent-orng --force
 *   bun run themes:port:opencode aura --out-dir /tmp/pollarys-theme-port-test --force
 ...
 *     or existing Pollarys theme JSON files
 *   - writes complete Pollarys light/dark theme JSON output with solid core
...
type ThemeSource = DesktopTheme | ContextThemeJson | PollarysTheme;
...
function isPollarysTheme(value: ThemeSource): value is PollarysTheme {
...
function syntheticTokensFromPollarysTheme(theme: PollarysTheme): ResolvedTheme {
...
async function main(): Promise<void> {
  ...
  const generated: Array<{ fileName: string; theme: PollarysTheme }> = [];
    ...
    if (isPollarysTheme(source)) {
      ...
      generated.push({
        ...
        theme: buildTheme(
          syntheticTheme,
          variant,
          syntheticTokensFromPollarysTheme(source),
